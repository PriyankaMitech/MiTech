                                                        ...
                                                        <?php if ($task->project_id == $project['projectId']): ?>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <input type="hidden" id="isStartTimeSet" value="<?= $isStartTimeSet ? '1' : '0' ?>">
...
//      Split the current time into date and time components
        ...
        // After making the AJAX call to start the task
// After making the AJAX call to start the task
$.ajax({
    url: '<?php echo base_url();?>record-action',
    method: 'POST',
    ...
    success: function(response) {
        // Check if the start time is set in the response
        var isStartTimeSet = response.allotTaskData && response.allotTaskData.start_time;

        // Set the value of the hidden field
        $('#isStartTimeSet').val(isStartTimeSet ? '1' : '0');

        // Hide or show the start button based on whether the start time is set
        if (isStartTimeSet) {
            $('.startBtn').hide();
        } else {
            $('.startBtn').show();
        ...
        // Handle other logic or UI updates based on the response
        console.log(response);
    },
    error: function(xhr, status, error) {
        // Handle error if needed
